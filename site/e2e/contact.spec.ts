...
test('Users can send an email', async ({ page, login }) => {
	...
	await expect(
		mainContent.getByText(
			/have a general question\? the best place is the call AbhiDev podcast\./i,
		),
	).toBeVisible()
	await expect(
		mainContent.getByRole('link', { name: /call AbhiDev podcast/i }),
	).toHaveAttribute('href', '/calls')
	await expect(mainContent.getByRole('textbox', { name: /name/i })).toHaveValue(
		user.firstName,
	)
	await expect(
		mainContent.getByRole('textbox', { name: /email/i }),
...
${bodyLorem}
  `.trim()
	...
	const email = await readEmail((em) => em.to.includes('me@abhidev.com'), {
		maxRetries: 10,
		retryDelay: 200,
	})
