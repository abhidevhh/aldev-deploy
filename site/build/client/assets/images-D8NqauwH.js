...
import { o as optionalTeams, t as toBase64 } from "./misc-DM3BUXHg.js";
setConfig({
  cloudName: "abhidev-com"
});
const createImages = (images2) => {
  const imageBuilders = {};
  for (const [name, { id, alt, className, style }] of Object.entries(images2)) {
    imageBuilders[name] = getImageBuilder(id, alt, { className, style });
  }
  return imageBuilders;
};
function getImageBuilder(id, alt = "", { className, style } = {}) {
...
const abhiBuddySnowboardingImages = {
...
const abhiBuddyOnewheelingImages = {
...
const abhiBuddyPlayingSoccerImages = {
...
const abhiBuddyBackFlippingImages = {
...
const abhiBuddyFlyingSnowboardingImages = {
...
const abhiBuddyFlyingSkiingImages = {
...
const abhiBuddyFlyingOnewheelingImages = {
...
const abhiBuddyFlyingPlayingSoccerImages = {
...
const abhiBuddyFlyingBackFlippingImages = {
...
function getRandomFlyingAbhiBuddy(team, randomImageNo = Math.random()) {
  const activities = [
  ...
  const set = activities[Math.floor(randomImageNo * activities.length)] ?? abhiBuddySnowboardingImages;
  if (team) {
    return set[team];
  } else {
    const randomTeam = optionalTeams[Math.floor(randomImageNo * optionalTeams.length)] ?? "UNKNOWN";
...
function getImgProps(imageBuilder, {
  widths,
  sizes,
  transformations,
  className,
  ...
  const aspectRatio = transformations?.resize?.aspectRatio ? transformations.resize.aspectRatio.replace(":", "/") : transformations?.resize?.height && transformations.resize.width ? `${transformations.resize.width}/${transformations.resize.height}` : imageBuilder.style?.aspectRatio;
  return {
    style: {
      ...imageBuilder.style,
      aspectRatio,
...
function emojiStrip(string) {
  return string.replace(emojiRegex(), "").split(" ").filter(Boolean).join(" ").trim();
}
...
export {
  getImageBuilder as a,
  getGenericSocialImage as b,
  getSocialImageWithPreTitle as c,
  abhiBuddyImages as d,
  abhiBuddyProfiles as e,
  getRandomFlyingAbhiBuddy as f,
  getImgProps as g,
  illustrationImages as h,
  images as i
};
//# sourceMappingURL=images-D8NqauwH.js.map
