...
export const oauthHandlers: Array<HttpHandler> = [
	// https://kcd-oauth-provider.abhidev.workers.dev/.well-known/oauth-authorization-server
	http.get<any, DefaultRequestMultipartBody>(
		...
		async ({ request }) => {
			...
			if (localIsRunning) {
				...
				// find/replace https://kcd-oauth-provider.abhidev.workers.dev/ with http://localhost:8787/ if local is running
				const body = await response.arrayBuffer()
				const text = new TextDecoder().decode(body)
				const newText = text.replaceAll(
					/https?:\/\/kcd-oauth-provider\.abhidev\.workers\.dev/g,
					'http://localhost:8787',
				)
	...
	http.all<any, DefaultRequestMultipartBody>(
		'https://kcd-oauth-provider.abhidev.workers.dev/*',
		async ({ request }) => {
			const localIsRunning = await isPortOpen('localhost', 8787)
