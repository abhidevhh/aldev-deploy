...
export const mermaidToSvgHandlers: Array<HttpHandler> = [
	http.get(
		'https://mermaid-to-svg.abhidev.workers.dev/svg',
		async ({ request }) => {
			...
			const theme =
				url.searchParams.get('theme') === 'dark' ? 'dark' : 'default'
			const mermaidString =
				lz.decompressFromEncodedURIComponent(compressed) ?? ''
			const snippet = escapeXml(
				mermaidString.replace(/\s+/g, ' ').slice(0, 160),
			)
