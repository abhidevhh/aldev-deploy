...
function isAbhiCallQueueMessage(value: unknown): value is {
	draftId: string
	callAudioKey: string
	responseAudioKey: string
} {
...
async function processAbhiCallAudioQueueMessage({
	draftId,
	callAudioKey,
	responseAudioKey,
}: {
...
function getStaticDocs(): SearchDoc[] {
	return [
		...
		},
		{
			...
			content: 'call AbhiDev Podcast episodes.',
		},
	]
}
...
export const cloudflareHandlers: Array<HttpHandler> = [
	http.post<any, DefaultBodyType>(
		...
		async ({ request }) => {
			...
			requiredHeader(request.headers, 'authorization')
			let payload: AbhiCallQueueEnvelope | null = null
			try {
				payload = (await request.json()) as AbhiCallQueueEnvelope
			} catch {
			...
			if (
				...
				!message ||
				!isAbhiCallQueueMessage(message)
			) {
				return jsonError(
					400,
			...
			void processAbhiCallAudioQueueMessage(message)
	...
	http.post<any, DefaultBodyType>(
		...
		async ({ request }) => {
			...
			// Text generation / chat models (used for call AbhiDev metadata generation).
			...
			if (hasMessages || hasPrompt) {
				...
				return jsonOk({
					response: JSON.stringify({
						title: `Mock call AbhiDev episode title (${model})`,
						description:
							'Mock description generated by Workers AI. This is a placeholder used in local mocks.',
						keywords: 'call AbhiDev, mock, podcast, workers ai, transcript',
					}),
				})
			}
