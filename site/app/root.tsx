...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
	...
	const title = 'Aldev'
	const description =
		'Come check out how Aldev can help you level up your career as a software engineer.'
	return [
		{ viewport: 'width=device-width,initial-scale=1,viewport-fit=cover' },
		{
			'theme-color':
				requestInfo?.userPrefs.theme === 'dark'
					? '#1F2028'
					: requestInfo?.userPrefs.theme === 'brand'
					? '#eef7ff'
					: '#FFF',
		},
		...getSocialMetas({
...
function kcdLiveReloadConnect(config?: { onOpen: () => void }) {
	...
	ws.onmessage = (message) => {
		...
		if (
			event.type === 'abhidev.com:file-change' &&
	...
	ws.onclose = (event) => {
		if (event.code === 1006) {
			console.log(
				'abhidev.com dev server web socket closed. Reconnecting...',
			)
			setTimeout(
				() =>
