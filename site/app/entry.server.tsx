...
const getEnv = serverOnly$(async () => {
	const path = ['.', '/', 'u', 't', 'i', 'l', 's', '/', 'e', 'n', 'v', '.', 's', 'e', 'r', 'v', 'e', 'r', '.', 't', 's'].join('')
	const env = await import(path)
	env.init()
	global.ENV = env.getPublicEnv()
	return env.getEnv
})

const getEnsurePrimary = serverOnly$(async () => {
	const path = ['.', '/', 'u', 't', 'i', 'l', 's', '/', 'l', 'i', 't', 'e', 'f', 's', '-', 'j', 's', '.', 's', 'e', 'r', 'v', 'e', 'r', '.', 't', 's'].join('')
	const { ensurePrimary } = await import(path)
...
export default async function handleDocumentRequest(...args: DocRequestArgs) {
	...
	if (responseStatusCode >= 500) {
		...
		// if it can handle it.
		const ensurePrimary = await getEnsurePrimary()
		await ensurePrimary()
	}

	for (const handler of await getOtherRoutes()) {
