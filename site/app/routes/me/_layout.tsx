...
export async function loader({ request }: Route.LoaderArgs) {
	...
	const [sessionCount, rawFavorites, abhiCallCallerEpisodes] =
		await Promise.all([
	...
	const wantsCallEpisodes = rawFavorites.some(
		(f) => f.contentType === 'abhi-call-episode',
	)
	const wantsCallEpisodeData =
		wantsCallEpisodes || abhiCallCallerEpisodes.length > 0
	const wantsChatEpisodes = rawFavorites.some(
		(f) => f.contentType === 'chats-with-abhi-episode',
	)
...
function YouScreen({ loaderData: data, actionData }: Route.ComponentProps) {
	...
	return (
		<main>
			...
			<Grid>
				...
				{data.favorites.length ? (
					...
					<Paragraph className="col-span-full">
						No favorites yet. Open a blog post, talk, podcast episode, or
						YouTube video and hit the star.
					</Paragraph>
				)}
			</Grid>
			...
			<Grid>
				<div className="col-span-full mb-12">
					<H2>Your call AbhiDev episodes</H2>
					<H2 variant="secondary" as="p">
						Episodes where you're the caller.
					</H2>
				</div>
				{data.abhiCallCallerEpisodes.length ? (
					<ul className="col-span-full space-y-4">
						{data.abhiCallCallerEpisodes.map((episode) => (
							<li
								key={episode.id}
								className="border-b border-gray-200 pb-4 dark:border-gray-600"
							>
