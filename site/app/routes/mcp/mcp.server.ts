...
import { type SearchResult } from '@abhidev/search-shared'
import { z } from 'zod'
...
import { getSeasons as getAbhiTalksSeasons } from '#app/utils/simplecast.server.js'
...
function createServer() {
	...
	server.registerTool(
		...
		{
			description:
				'Get the details (title, description, transcript, etc.) for a specific episode of the Engineering with AbhiDev podcast by its season number and episode number',
			inputSchema: {
				seasonNumber: z
					.number()
		...
		async ({ episodeNumber, seasonNumber }) => {
			...
			const seasons = await getAbhiTalksSeasons({ request })
				...
				content: [
					...
					},
					{
						...
						text: episode.transcriptHTML
							...
							: `Transcript: No transcript found for ${episode.title} (Engineering with AbhiDev S${seasonNumber}E${episodeNumber})`,
					},
				],
			}
		},
	...
	server.registerTool(
		...
		{
			description:
				'Subscribe to AbhiDev newsletter and get regular updates about new articles and courses',
			inputSchema: {
				email: z
					.email()
					.optional()
