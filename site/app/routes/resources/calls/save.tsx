...
import { getPublishedAbhiCallEpisodeEmail } from '#app/utils/abhi-call-published-email'
import {
	getErrorForAudio,
	getErrorForTitle,
	getErrorForNotes,
	...
	try {
		...
		try {
			await prisma.$transaction([
				prisma.abhiCallCallerEpisode.create({
					data: {
						userId: call.userId,
						callTitle,
						callNotes,
...
async function createEpisodeDraft({
	...
	const [, draft] = await prisma.$transaction([
		prisma.abhiCallEpisodeDraft.deleteMany({ where: { callId } }),
...
async function generateCallerTranscript({
	...
	void startAbhiCallCallerTranscriptProcessing(callId, { force: true })
	return redirect(`/calls/admin/${callId}`)
}
...
async function undoEpisodeDraft({
	...
	await prisma.abhiCallEpisodeDraft.deleteMany({ where: { callId } })
	return redirect(`/calls/admin/${callId}`)
}
