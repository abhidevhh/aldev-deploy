...
import {
	...
	abhiCallTextToSpeechVoices,
	type AbhiCallTextToSpeechVoice,
...
export function AbhiCallTextToSpeech({
	onAcceptAudio,
}: {
	onAcceptAudio: (args: {
		audio: Blob
		questionText: string
		voice: AbhiCallTextToSpeechVoice
	}) => void
}) {
	...
	const defaultVoice = (abhiCallTextToSpeechVoices[0]?.id ??
		'luna') as AbhiCallTextToSpeechVoice
	const [voice, setVoice] =
		React.useState<AbhiCallTextToSpeechVoice>(defaultVoice)
	...
	async function saveQuestion() {
		...
		const validationError = getErrorForAbhiCallQuestionText(
		...
		try {
			...
			try {
				...
				if (
					...
					durationSeconds >
						abhiCallTextToSpeechConstraints.maxAudioDurationSeconds
				) {
					setServerError(
						`Generated audio is longer than ${abhiCallTextToSpeechConstraints.maxAudioDurationSeconds}s. Please shorten your question and try again.`,
					)
					return
				}
			} finally {
					...
					<div className="flex flex-col gap-3 sm:flex-row sm:items-end">
						<select
							...
							onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
								setVoice(e.currentTarget.value as AbhiCallTextToSpeechVoice)
							}
						>
							{abhiCallTextToSpeechVoices.map((v) => (
