...
export const meta: Route.MetaFunction = ({ data, matches }) => {
	...
	if (!episode) {
		return [{ title: 'Engineering with AbhiDev Episode not found' }]
	}
	const {
		description,
		image,
		...
		...getSocialMetas({
			...
			image: getSocialImageWithPreTitle({
				...
				url: getDisplayUrl({
					...
					path: getAbhiEpisodePath({ seasonNumber, episodeNumber }),
				}),
			}),
		}),
		{ 'twitter:card': 'player' },
...
export async function loader({ request, params }: Route.LoaderArgs) {
	...
	const favoriteContentType = 'chats-with-abhi-episode' as const
							...
							href={`https://x.com/intent/post?${new URLSearchParams({
								...
								text: `I just listened to "${episode.title}" with ${listify(
									episode.guests
										.map((g) => (g.x ? `@${g.x}` : null))
										.filter(typedBoolean),
								)} on the call AbhiDev Podcast 🎙 by @abhidev`,
							})}`}
						>
