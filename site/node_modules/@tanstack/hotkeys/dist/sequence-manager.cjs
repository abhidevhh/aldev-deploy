...
var SequenceManager = class SequenceManager {
	...
	#platform;
	constructor() {
		this.registrations = new _tanstack_store.Store(/* @__PURE__ */ new Map());
	...
	*/
	register(sequence, callback, options = {}) {
		...
		if (conflictingRegistration) require_manager_utils.handleConflict(conflictingRegistration.id, require_format.formatHotkeySequence(sequence), conflictBehavior, (regId) => this.#unregister(regId));
		const firstStep = parsedSequence[0];
		const resolvedIgnoreInputs = options.ignoreInputs ?? require_manager_utils.getDefaultIgnoreInputs(firstStep);
		const registration = {
			id,
			sequence,
			parsedSequence,
	...
	#processTargetEvent(event, target, eventType) {
		...
		for (const id of targetRegs) {
			...
			if (registration.options.ignoreInputs !== false) {
				if (require_manager_utils.isInputElement(event.target)) {
...
//# sourceMappingURL=sequence-manager.cjs.map