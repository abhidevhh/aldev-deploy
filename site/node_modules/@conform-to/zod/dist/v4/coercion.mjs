...
function pipeWithOptionality(coercion, target) {
  var schema = pipe(transform(coercion), target);
...
function withOptionalInput(type) {
  ...
  var _Object$getOwnPropert = Object.getOwnPropertyDescriptors(type),
    descriptors = _objectWithoutProperties(_Object$getOwnPropert, _excluded);
  ...
  if (coercion) {
    ...
    schema = pipe(transform(value => {
      ...
      options: def.options.map(item => {
        ...
        var innerDef = options.skipValidation ? _objectSpread2(_objectSpread2({}, objectDef), {}, {
          catchall: undefined
        }) : objectDef;
