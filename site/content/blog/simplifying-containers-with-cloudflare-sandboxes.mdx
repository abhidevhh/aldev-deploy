...
That spike became [PR #726](https://github.com/abhidev/abhidev.com/pull/726).
...
The design in that PR was: a dedicated `abhi-call-audio-sandbox` service with
...
into [PR #729](https://github.com/abhidev/abhidev.com/pull/729). It took a
...
sequenceDiagram
  ...
  Worker->>Sandbox: exec(abhi-call-audio-cli, env={signed URLs})
...
WORKDIR /opt/abhi-call-audio

COPY assets ./assets
COPY sandbox/abhi-call-audio-cli.sh /usr/local/bin/abhi-call-audio-cli

RUN chmod +x /usr/local/bin/abhi-call-audio-cli
```
...
```ts
const completed = await runAbhiCallAudioSandboxJob({
	binding: env.Sandbox,
...
And `runAbhiCallAudioSandboxJob` boils down to:

```ts
const sandbox = getSandbox(binding, sandboxId)
try {
	const result = await sandbox.exec('/usr/local/bin/abhi-call-audio-cli', {
...
```ts
const sandboxId = `abhi-call-audio-${draftId}-${crypto.randomUUID()}`
```
...
`abhi-call-` is 10 characters, each compact segment is 12, the separator is 1,
