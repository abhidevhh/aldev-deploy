...
date: 2026-03-10
description:
  'The interesting part of moving abhidev.com to npm workspaces was not the
...
For a while, abhidev.com had two separate deployable things living in the
same git repo:
...
- A **call AbhiDev audio worker** (Cloudflare Worker)
- A **call AbhiDev audio container** (separate Docker container)
...
services/
  ...
  abhi-call-audio-worker/     ← Cloudflare audio worker
  abhi-call-audio-container/  ← Docker audio container
```
...
```json
{
	...
	"workspaces": ["services/*"],
	"scripts": {
		"dev": "npm run dev --workspace abhidev.com",
		"build": "npm run build --workspace abhidev.com",
		"typecheck": "npm run typecheck --workspace abhidev.com",
...
Three old nested lockfiles (`abhi-call-audio-container/package-lock.json`,
`abhi-call-audio-worker/package-lock.json`, `oauth/package-lock.json`) were
...
- name: 📥 Install site deps
  run: npm ci --workspace=abhidev.com
```
...
- name: 🌐 Install Playwright browsers
  ...
  run: npm run test:e2e:install --workspace abhidev.com
```
