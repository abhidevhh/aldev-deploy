...
date: 2026-03-09
description:
  'How I moved call AbhiDev podcast episode processing off my primary Fly.io app
...
For 226 episodes of my [call AbhiDev podcast](/calls), I recorded audio, combined
it with my response audio, and ran the whole thing through FFmpeg directly on
the same Fly.io machine that serves abhidev.com. It was a simple pipeline
...
[an extra long recording](https://abhidev.com/calls/05/26/what-to-learn) a
call AbhiDev episode and hit publish. My app runs FFmpeg during the publish flow:
...
The Fly.io instance running abhidev.com hit extreme CPU saturation. The
metrics tell the story clearly:

![Fly.io instance metrics from March 6, 2026. Load average spikes to 400–500%. CPU Utilization is saturated. The CPU Quota Balance and Throttling graph shows the machine hitting its CPU quota ceiling.](https://res.cloudinary.com/abhidev-com/image/upload/v1773089500/abhidev.com/content/blog/offloading-ffmpeg-with-cloudflare/fly-metrics-march-6-problematic-cpu)
...
started building the call AbhiDev feature, I could have designed a proper job
...
abhidev.com runs on Fly.io with a primary instance and read replicas. The
...
When a call AbhiDev episode is submitted, the app enqueues a job to a Cloudflare
...
```ts
// app/utils/abhi-call-audio-processor.server.ts
const res = await fetch(
	`${env.ABHI_CALL_AUDIO_CF_API_BASE_URL}/accounts/${env.CLOUDFLARE_ACCOUNT_ID}/queues/${queueId}/messages`,
	{
		method: 'POST',
		headers: {
...
if (!verifyAbhiCallAudioProcessorCallbackSignature(signature, rawBody)) {
	return new Response('Invalid signature', { status: 401 })
}
const event = parseAbhiCallAudioProcessorEvent(rawBody)
...
![Fly.io instance metrics from March 9, 2026 during the new offloaded FFmpeg run. Load average peaks around 60–80%. Memory is stable. No CPU throttling visible.](https://res.cloudinary.com/abhidev-com/image/upload/v1773089500/abhidev.com/content/blog/offloading-ffmpeg-with-cloudflare/fly-metrics-march-9-offloaded-cpu)
...
#720](https://github.com/abhidev/abhidev.com/pull/720) in the public
abhidev.com repo.
